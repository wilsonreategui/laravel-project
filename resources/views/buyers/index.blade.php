    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                {{-- Botón Crear nuevo Buyer --}}
                <div style="display: flex; justify-content: center; padding-top: 20px; ">
                    <a href="{{route('buyers.create')}}" style="background: #6875F5; padding: 10px; color: white; border-radius: 15px;">New Buyer</a>
                </div>
                                    ...
                                    <tr class="text-gray-700">
                                        <td class=" border">
                                            <div class="flex items-center text-sm" style="justify-content: center;">
                                                <div>
                                                    <p class="font-semibold text-black my-0">{{$buyer->id}}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class=" text-ms font-semibold border">{{$buyer->name}}</td>
                                        <td class=" text-sm border">{{$buyer->email}}</td>
                                        <td class=" text-sm border">
                                            <div class="justify-center my-8 select-none flex">
                                                <a href="{{route('buyers.edit', ['buyer' => $buyer])}}" class="px-3 ">Edit</a>
                                                
                                                <form action="{{ route('buyers.destroy', ['buyer' => $buyer])}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
